...
numerical_cols = ['LotFrontage', 'LotArea', 'YearBuilt', 'YearRemodAdd', 'MasVnrArea', 'BsmtFinSF1',
       ...
       'MiscVal', 'MoSold', 'YrSold', "Age_House", "Yrs_Since_Remodel", "Age_Garage", "SalePrice"]
...
# Continuous numerical variables transformation
def log_transform(col):
    ...
    plt.savefig(f"plots/log_transformed_{col}.png")
    plt.close()

    plt.figure(figsize=(10, 5))
    sns.histplot(train_after[f"Log_{col}"], kde=True)
    plt.title(f"Histogram and Density Plot of Log_{col}")
    plt.savefig(f"plots/histo_density_after_log_{col}.png")
    plt.close()
...
def sqrt_transform(col):
    ...
    plt.savefig(f"plots/sqrt_transformed_{col}.png")
    plt.close()

    plt.figure(figsize=(10, 5))
    sns.histplot(train_after[f"Sqrt_{col}"], kde=True)
    plt.title(f"Histogram and Density Plot of Sqrt_{col}")
    plt.savefig(f"plots/histo_density_after_sqrt_{col}.png")
    plt.close()
...
def yeo_johnson_transform(col, df):
    ...
    plt.savefig(f"plots/Yeo-Johnson_transformed_{col}.png")
    plt.close()
    ...
    plt.savefig(f"plots/histo_density_after_Yeo-Johnson_{col}.png")
    plt.close()
